...
export async function POST(
  ...
  if (!pageUrl) {
    return NextResponse.json({ error: "URL da pagina e obrigatoria" }, { status: 400 })
  }
...
- Se nao conseguir verificar algo, use warning com descricao explicando`
    ...
    return NextResponse.json({ 
      error: `Erro ao auditar pagina: ${err instanceof Error ? err.message : "Erro desconhecido"}` 
    }, { status: 500 })
  }
}
